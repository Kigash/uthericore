page 50979 "Transfer Files Card"
{
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Carried By(Name)"; Rec."Carried By(Name)")
                {
                    Editable = false;
                    Visible = false;
                    ApplicationArea = All;
        ...
        area(creation)
        {
            action("Transfer File")
            {
                ...
                begin
                    ...
                    TransferFilesLines.SETRANGE("Transfer ID", Rec."Transfer ID");
                    ...
                    IF CONFIRM('Are you sure you want to transfer file?') THEN BEGIN
                        ...
                        IF Rec.Status = Rec.Status::New THEN BEGIN
                            Rec.Status := Rec.Status::"Pending Receipt";
                        ...
                        User.RESET;
                        User.GET(Rec."Released To");
                        mailheader := 'NOTIFICATION OF FILE TRANSFER';
                        mailbody := 'Dear ' + Rec."Released To" + ',<br><br>';
                        mailbody := mailbody + 'Please Note that ' + Rec."Released From" + ' has transferred the following files to you' + Rec."Released To" + ' under transfer ID:<b>' + Rec."Transfer ID" + '</b> <br><br>';
                        TransferFilesLines.RESET;
                        TransferFilesLines.SETRANGE("Transfer ID", Rec."Transfer ID");
            ...
            action("Receive File")
            {
                ...
                trigger OnAction();
                begin
                    Rec.TestField("Duration Required(Days)");
                    User.GET(USERID);
                    IF User."User ID" <> Rec."Released To" THEN BEGIN
                        ERROR('You cannot receive the file transferred to %1.', Rec."Released To");
                    END;
                    IF CONFIRM('Are you sure you want to receive the file(s)?') THEN BEGIN
                        ...
                        IF TransferFilesLines.FIND('-') THEN BEGIN
                            REPEAT
                                ...
                                TransferFilesLines."Carried By" := Rec."Released To";
                        ...
                        TransferFilesLines.RESET;
                        TransferFilesLines.SETRANGE("Received By", Rec."Released From");
                        ...
                        IF TransferFilesLines.FIND('-') THEN BEGIN
                            REPEAT
                                ...
                                TransferFilesLines."Returned by" := Rec."Released From";
                                TransferFilesLines.MODIFY;
                            UNTIL
                              TransferFilesLines.NEXT = 0;
                        END;
                        ...
                        IF Rec.Status = Rec.Status::"Pending Receipt" THEN
                        ...
                        mailbody := 'Dear ' + Rec."Released From" + ',<br><br>';
                        mailbody := mailbody + 'Please Note that ' + Rec."Released To" + ' has confirmed receipt of the following files transferred under transfer ID:<b>' + Rec."Transfer ID" + '</b> by <b>' + Rec."Released From" + '</b> <br><br>';
                        TransferFilesLines.RESET;
                        TransferFilesLines.SETRANGE("Transfer ID", Rec."Transfer ID");
                        IF TransferFilesLines.FINDSET THEN BEGIN
                            REPEAT
            ...
            action("Cancel File Transfer")
            {
                ...
                trigger OnAction();
                begin
                    IF Rec.Status = Rec.Status::Received THEN BEGIN
                        ERROR('File has already been received! You cannot cancel this transfer.');
                    END;

                    IF Rec."Released From" <> USERID THEN BEGIN
                    ...
                    IF CONFIRM('Are you sure you want to cancel this file transfer') THEN BEGIN
                        ...
                        TransferRegistryFiles.SETRANGE("Transfer ID", Rec."Transfer ID");
    ...
    begin
        ...
        END;
        ReceiveFile := FALSE;
        IF Rec.Status = Rec.Status::"Pending Receipt" THEN BEGIN
            IF USERID <> Rec."Released To" THEN BEGIN
                ReceiveFile := FALSE;
                DueDate := FALSE;
            END;
            IF USERID = Rec."Released To" THEN BEGIN
