page 51008 "Vehicle Maintenance Card"
{
    // version TL2.0

    PageType = Card;
    SourceTable = "Vehicle Maintenance";

    layout
    {
        area(content)
        {
            group("Vehicle Maintenance Scheduling")
            {
                field("Vehicle Number Plate"; Rec."Vehicle Number Plate")
                {
                    ApplicationArea = All;
                    TableRelation = "Vehicle Register";

                    trigger OnValidate();
                    begin
                        IF VehicleRegister.GET(Rec."Vehicle Number Plate") THEN BEGIN
                            Rec."Responsible Driver" := VehicleRegister."Responsible Driver";
                            Rec."Branch Code" := VehicleRegister."Branch Code";
                        END;
                    end;
                }
                field("Responsible Driver"; Rec."Responsible Driver")
                {
                    ApplicationArea = All;
                    TableRelation = "Driver Setup";
                }
                field("Branch Code"; Rec."Branch Code")
                {
                    ApplicationArea = All;
                }
                field("Service Date"; Rec."Service Date")
                {
                    ApplicationArea = All;
                }
                field("Service Agent Name"; Rec."Service Agent Name")
                {
                    ApplicationArea = All;
                }
                field(Comment; Rec.Comment)
                {
                    ApplicationArea = All;
                }
            }
        }
        area(factboxes)
        {
            systempart(Outlook; Outlook)
            {
                ApplicationArea = All;
            }
            systempart(Notes; Notes)
            {
                ApplicationArea = All;
            }
            systempart(MyNotes; MyNotes)
            {
                ApplicationArea = All;
            }
            systempart(Links; Links)
            {
                ApplicationArea = All;
            }
        }
    }

    actions
    {
        area(creation)
        {
            action(Schedule)
            {
                ApplicationArea = all;
                Caption = 'Schedule';
                Image = SignUp;
                Promoted = true;
                PromotedCategory = Process;
                PromotedIsBig = true;
                PromotedOnly = true;

                trigger OnAction();
                begin
                    AdminManagement.ScheduleVehicleMaintenance(Rec);
                    CurrPage.CLOSE;
                end;
            }
        }
    }

    var
        VehicleRegister: Record "Vehicle Register";
        AdminManagement: Codeunit "Admin Management";
}

