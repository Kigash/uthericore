        ...
        area(processing)
        {
            action("Upload Separation Form")
            {
                ...
                begin
                    ...
                    //MESSAGE('%1',selected);
                    docname := Rec."Employee No";
                    docname := CONVERTSTR(docname, '/', '_');
                    docname := CONVERTSTR(docname, '\', '_');

                    HRSetup.RESET;
                    HRSetup.GET;
                    //// HRSetup.TESTfield("File Path"); Rec.//FileName := filecu.OpenFileDialog('Select the Exit form', 'Exit', 'PDF Files (*.PDF)|*.PDF|All Files (*.*)|*.*');
                    ...
                    EmployeeDocuments."Employee No" := Rec."Employee No";
                    EmployeeDocuments."Employee Name" := Rec."Employee Name";
            ...
            action("View Document")
            {
                ...
                begin
                    IF CONFIRM('Are you sure you want to view this file?') THEN BEGIN
                        EmployeeDocuments.RESET;
                        EmployeeDocuments.SETRANGE("Employee No", Rec."Employee No");
