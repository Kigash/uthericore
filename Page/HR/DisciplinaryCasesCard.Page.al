    ...
    actions
    {
        area(processing)
        {
            action("Proceed To Ongoing")
            {
                ...
                trigger OnAction();
                begin
                    IF Rec."HOD Recommendation" = '' THEN BEGIN
            ...
            action("Attach HOD Recommendation")
            {
                ...
                begin
                    ...
                    IF filename <> '' THEN BEGIN
                        ...
                        Rec."HOD File Path" := HRsetup."File Path" + FileCU.GetFileName(filename);
            ...
            action("Attach HR Recommendation")
            {
                ...
                begin
                    ...
                    IF filename <> '' THEN BEGIN
                        ...
                        filename2 := CONVERTSTR(filename2, ' ', '_');
                        //FileCU.CopyClientFile(filename, HRsetup."File Path" + FileCU.GetFileName(filename), TRUE);
                        Rec."HR File Path" := HRsetup."File Path" + FileCU.GetFileName(filename);
                        MESSAGE('File Attached Successfully');
                    END;
                end;
            }
            action("Attach Commitee Recommendation")
            {
                ...
                begin
                    ...
                    IF filename <> '' THEN BEGIN
                        ...
                        filename2 := CONVERTSTR(filename2, ' ', '_');
                        //FileCU.CopyClientFile(filename, HRsetup."File Path" + FileCU.GetFileName(filename), TRUE);
                        Rec."Committe File Path" := HRsetup."File Path" + FileCU.GetFileName(filename);
                        MESSAGE('File Attached Successfully');
                    END;
                end;
            }
            action("Attach File After Appeal")
            {
                ...
                begin
                    ...
                    IF filename <> '' THEN BEGIN
                        ...
                        filename2 := CONVERTSTR(filename2, ' ', '_');
                        //FileCU.CopyClientFile(filename, HRsetup."File Path" + FileCU.GetFileName(filename), TRUE);
                        Rec."Committee File-After Appeal" := HRsetup."File Path" + FileCU.GetFileName(filename);
                        MESSAGE('File Attached Successfully');
                    END;
                end;
            }
