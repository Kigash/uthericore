page 55036 "Group Allocation Subform"
{
    ...
    actions
    {
        area(processing)
        {
            action("Get Collection Entries")
            {
                ...
                trigger OnAction()
                begin
                    IF GroupAllocationHeader.GET(Rec."Document No.") THEN
    ...
    begin
        IF GroupAllocationHeader.GET(Rec."Document No.") THEN;
        IF Rec."Transaction Date" < GroupAllocationHeader."Last Meeting Date" THEN
            StyleText := 'Unfavorable'
        ELSE
            StyleText := 'Standard';
        Rec.CalcFields("Allocated Amount");
        IF (Rec."Amount to Allocate" - Rec."Allocated Amount") = 0 THEN
            StyleText := 'Favorable'
    end;
}
