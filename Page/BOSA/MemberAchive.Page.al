page 59230 "Member Achive"
{
    // version TL2.0
    Caption = 'Member Archive';
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Office Collection"; Rec."Office Collection")
                {

                }
                field("Field Collection"; Rec."Field Collection")
                {

                }
                field("Christmas Savings"; Rec."Christmas Savings")
                { }
        ...
        area(processing)
        {
            action(SendApprovalRequest)
            {
                ...
                begin
                    ...
                    //ValidateAttachment;
                    IF ApprovalsMgmt.CheckMemberArchiveApprovalPossible(Rec) THEN
                        ApprovalsMgmt.OnSendMemberArchiveForApproval(Rec);
                    CurrPage.CLOSE;
                end;
            }
            action(CancelApprovalRequest)
            {
                ...
                var
                    ...
                    ApprovalsMgmt: Codeunit "Approvals Mgmt Ext";
                begin
                    ApprovalsMgmt.OnCancelMemberArchiveApprovalRequest(Rec);
                end;
            }
            action(Approve)
            {
            ...
            action(PostExit)
            {
                ...
                begin
                    if Confirm('Are You Sure You Want to Archive This Member', true) then begin
                        Bosa.ProcessMemberArchive(Rec);
                        CurrPage.Close();
                    end;
                end;
            }
