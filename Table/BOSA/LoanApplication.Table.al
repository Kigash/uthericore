table 50101 "Loan Application"
{
    ...
    fields
    {
        ...
        field(2; "Member No."; Code[20])
        {
            ...
            ELSE
            IF ("Member Category" = FILTER(Member)) Member Where(Status = filter(Active | Dormant));
            ...
            begin
                ...
                IF "Member Category" = "Member Category"::Member THEN BEGIN
                    ...
                    //Message('Fosa Ac %1', FosaM.GetOrdinaryMemberAccount(Member));
        ...
        field(17; "Top-up"; Boolean)
        {
            trigger OnValidate()
            begin
                If "Top-up" = false then begin
                    if GetNoofAllExistingLoan("Loan Product Type") > 0 then
        ...
        field(41; "Disbursal Date"; Date)
        {
            ...
            begin
                ...
                if "Repayment Frequency" = "Repayment Frequency"::Quarterly then
                    "Next Due Date" := CalcDate('3M', "Disbursal Date");
                if "Repayment Frequency" = "Repayment Frequency"::Annually then
                    "Next Due Date" := CalcDate('1Y', "Disbursal Date");*/
        ...
        {
            FieldClass = FlowField;
            CalcFormula = Sum("Detailed Cust. Ledg. Entry".Amount WHERE("Customer No." = FIELD("No."), "Entry Type" = filter("Initial Entry"), "Posting Date" = field("Date Filter")));
            Editable = false;
        ...
        field(123; "Pause Loan Interest"; Boolean)
        {
            trigger OnValidate()
            var
            ...
            begin
                if UserSetup.Get(UserId) then begin
                    if UserSetup."Pause Loan Charges" = false then
                        error('You dont have permission to pause loan charges. Kindly contact Admin.');

                    Clear("Int Paused By");
                    Clear("Int Paused Date");
                    Clear("Int Paused Time");
                    Clear("Int Paused By Host IP");
                    Clear("Int Paused By Host MAC");
                    Clear("Int Paused By Host Name");
                    If "Pause Loan Interest" = true then begin
                        GlobalManagement.GetHostInfo(HostName, HostIP, HostMac);
                        "Int Paused By" := USERID;
                        "Int Paused Date" := TODAY;
                        "Int Paused Time" := TIME;
                        "Int Paused By Host IP" := HostIP;
                        "Int Paused By Host MAC" := HostMac;
                        "Int Paused By Host Name" := HostName;
                    end;
                end;
            end;
        }
        field(124; "Pause Loan Penalty"; Boolean)
        {
            trigger OnValidate()
            var
                UserSetup: Record "User Setup";
            begin
                if UserSetup.Get(UserId) then begin
                    if UserSetup."Pause Loan Charges" = false then
                        error('You dont have permission to pause loan charges. Kindly contact Admin.');
                    Clear("Pen Paused By");
                    Clear("Pen Paused Date");
                    Clear("Pen Paused Time");
                    Clear("Pen Paused By Host IP");
                    Clear("Pen Paused By Host MAC");
                    Clear("Pen Paused By Host Name");
                    If "Pause Loan Penalty" = true then begin
                        GlobalManagement.GetHostInfo(HostName, HostIP, HostMac);
                        "Pen Paused By" := USERID;
                        "Pen Paused Date" := TODAY;
                        "Pen Paused Time" := TIME;
                        "Pen Paused By Host IP" := HostIP;
                        "Pen Paused By Host MAC" := HostMac;
                        "Pen Paused By Host Name" := HostName;
                    end;
                end;
            end;
        }
        field(125; "Int Paused By Host Name"; Code[30])
        {
            Editable = false;
        }
        field(126; "Int Paused By Host IP"; Code[20])
        {
            Editable = false;
        }
        field(127; "Int Paused By Host MAC"; Code[30])
        {
            Editable = false;
        }
        field(128; "Int Paused By"; Code[30])
        {
            Editable = false;
        }
        field(129; "Int Paused Date"; Date)
        {
            Editable = false;
        }
        field(130; "Int Paused Time"; Time)
        {
            Editable = false;
        }
        field(131; "Pen Paused By Host Name"; Code[30])
        {
            Editable = false;
        }
        field(132; "Pen Paused By Host IP"; Code[20])
        {
            Editable = false;
        }
        field(133; "Pen Paused By Host MAC"; Code[30])
        {
            Editable = false;
        }
        field(134; "Pen Paused By"; Code[30])
        {
            Editable = false;
        }
        field(135; "Pen Paused Date"; Date)
        {
            Editable = false;
        }
        field(136; "Pen Paused Time"; Time)
        {
            Editable = false;
        }
        field(137; "Guarantors Notified"; Boolean)
        {
            Editable = false;
        }
        field(138; "Loan Officer"; Code[30])
        {
            TableRelation = "User Setup";
        }
        field(139; "Expected Balance"; Decimal)
        {
            Editable = false;
        }
        field(50000; "Schedule Corrected"; Boolean)
        {
    ...
    begin
        ...
        IF LoanProductType.GET("Loan Product Type") THEN BEGIN
            ...
            if GetNoofAllExistingLoan("Loan Product Type") > 0 then begin
                "Top-up" := true;
                Validate("Top-up");
            end else
                "Top-up" := false;
            if LoanProductType."Board/Staff" = true then begin


                member.Reset();
                member.SetRange("No.", Rec."Member No.");
                if member.FindFirst() then begin
                    //   Message('%1, %2', member."No.", member."Sub Category");
                    if (member."Sub Category" = member."Sub Category"::" ") then
                        Error(Error015);
    ...
    local procedure GetNoofAllExistingLoan(Product: Code[100]): Integer
    var
        NoofLoans: Integer;
    begin
        NoofLoans := 0;
        Customer.RESET;
        Customer.SETRANGE("Member No.", "Member No.");
        Customer.SetRange("Customer Posting Group", Product);
        if Customer.FindSet() then begin
            repeat
                Customer.CalcFields("Balance (LCY)");
                if Customer."Balance (LCY)" > 0 then
    ...
    var
        ...
        Error015: Label 'Product intended for board or staff';
        MembershipDate: Date;
        ...
        NotActiveErr: Label '%1 %2 member status is %3';
